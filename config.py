...
def submit(file_paths):
    # Check if any file paths are empty
    if any(not path.get() for path in file_paths):
        messagebox.showerror("Error", "Please select both file paths.")
    else:
...
def run_config():
    ...
    # Submit Button
    tk.Button(root, text="Submit", command=submit(file_paths), font=("Arial", 12, "bold")).grid(row=3, columnspan=3, pady=20)

    # Hint
    tk.Label(root, text="The default locations are:", font=("Arial", 10), fg="gray").grid(row=4, columnspan=3,
                                                                                          pady=(0, 10))
    tk.Label(root, text=github_desktop_path, font=("Arial", 10), fg="gray").grid(row=5, columnspan=3)
    tk.Label(root, text=git_path, font=("Arial", 10), fg="gray").grid(row=6, columnspan=3)
